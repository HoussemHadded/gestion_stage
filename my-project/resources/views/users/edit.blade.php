                ...
                <form action="{{ route('admin.users.update', $user->id) }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="role" id="role"
                            ...
                            <option value="etudiant"   {{ old('role', $user->role->value) === 'etudiant'   ? 'selected' : '' }}>Étudiant</option>
                            <option value="entreprise" {{ old('role', $user->role->value) === 'entreprise' ? 'selected' : '' }}>Entreprise</option>
                            <option value="encadrant"  {{ old('role', $user->role->value) === 'encadrant'  ? 'selected' : '' }}>Encadrant</option>
                        </select>
                        @error('role')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
